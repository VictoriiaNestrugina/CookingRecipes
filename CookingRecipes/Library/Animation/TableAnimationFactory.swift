...
//  CookingRecipes
//
//  Created by Victoriia Nestrugina on 7/25/21.
//
...
enum TableAnimationFactory {
    ...
    // fades the cell by setting alpha as zero and moves the cell downwards,
    // then animates the cell's alpha and returns it to it's original position based on indexPaths
    ...
    // moves the cell downwards, then animates the cell's by returning them to their original position
    // based on indexPaths
    static func makeMoveUpAnimation(rowHeight: CGFloat,
                                    duration: TimeInterval,
                                    delayFactor: TimeInterval) -> TableCellAnimation {
        return { cell, indexPath, _ in
            cell.transform = CGAffineTransform(translationX: 0, y: rowHeight * 1.4)
    ...
    // moves the cell downwards, then animates the cell's by returning them to their original position
    // with spring bounce based on indexPaths
    static func makeMoveUpBounceAnimation(rowHeight: CGFloat,
                                          duration: TimeInterval,
